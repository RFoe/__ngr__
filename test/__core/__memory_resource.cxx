...
void test_acquire_fields() {
    __shared_block_subrange s = __acquire(128);
...
void test_coalescing() {
    __shared_block_subrange a = __acquire(64);
    __shared_block_subrange b = __acquire(64);
...
void test_block_boundary() {
    ...
    if (remaining == 0) {
        ...
        __shared_block_subrange s = __acquire(1);
        assert(s.__source_ != nullptr);
        assert(s.__offset_ == 0);
        __release(s);
        return;
    }
    __shared_block_subrange a = __acquire(remaining); // fills current block exactly
    __shared_block_subrange b = __acquire(1);         // must spill to a new block
...
void test_refcount() {
    __shared_block_subrange a          = __acquire(32);
    __PTRDIFF_TYPE__        count_base = a.__source_->__use_count_;

    __shared_block_subrange b = __acquire(32); // same block
    assert(b.__source_ == a.__source_);
    ...
    for (int i = 0; i < kThreads; ++i) {
        workers.emplace_back([&, i] {
            for (int j = 0; j < kIters; ++j) {
                __shared_block_subrange s = __acquire(64);
                if (s.__source_ == nullptr || s.__len_ != 64)
                    errors.fetch_add(1, std::memory_order_relaxed);
                for (__u32 __n = 0; __n < s.__len_; ++__n) {
...
void test_remote_round_trip() {
    ...
    std::vector<__shared_block_subrange> spans;

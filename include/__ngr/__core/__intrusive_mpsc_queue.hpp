...
template <typename _Node, std::atomic<_Node *> _Node::*_Next>
    requires std::default_initializable<_Node>
struct __intrusive_mpsc_queue<_Next> {
    std::atomic<_Node *> __head_{&__nil_};
    _Node               *__tail_{&__nil_};
    _Node                __nil_{};

    __intrusive_mpsc_queue() noexcept {
        (__nil_.*_Next).store(nullptr, std::memory_order_release);
    }

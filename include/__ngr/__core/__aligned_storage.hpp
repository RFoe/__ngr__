...
template <typename _Ty> struct __aligned_storage {
    ...
    inline constexpr auto _M_construct(_Args &&...__args)
        noexcept(std::is_nothrow_constructible_v<_Ty, _Args...>) -> _Ty & {
        return *std::launder(::new (static_cast<void *>(__buf_))
                                 _Ty{static_cast<_Args &&>(__args)...});
    }
    [[__gnu__::__always_inline__, __gnu__::__artificial__]]
    inline constexpr void _M_destroy() noexcept {
        std::destroy_at(&_M_get());
    }
    constexpr auto _M_get() & noexcept -> _Ty & {
        return *reinterpret_cast<_Ty *>(__buf_);
    }
    constexpr auto _M_get() && noexcept -> _Ty && {
        return static_cast<_Ty &&>(*reinterpret_cast<_Ty *>(__buf_));
    }
    constexpr auto _M_get() const & noexcept -> _Ty const & {
        return *reinterpret_cast<_Ty const *>(__buf_);
    }
    ...
    alignas(_Ty) unsigned char __buf_[sizeof(_Ty)]{};
};
...
struct __aligned_storage<_Ref> {
    [[__gnu__::__always_inline__, __gnu__::__artificial__]]
    inline constexpr auto _M_construct(_Ref __ref) noexcept -> _Ref {

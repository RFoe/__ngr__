...
inline auto __io_uring_setup(unsigned __entries, ::io_uring_params &__params)
    -> __file_descriptor {
    int __rc = static_cast<int>(::syscall(__NR_io_uring_setup, __entries, &__params));
...
inline auto __io_uring_enter(
    ...
    int __rc = static_cast<int>(::syscall(
        __NR_io_uring_enter, __ring_fd, __to_submit, __min_complete, __flags, nullptr,
        0));
...
inline auto __duration_to_timespec(std::chrono::nanoseconds __dur) noexcept
    ...
    __dur = std::clamp(
        __dur, std::chrono::nanoseconds{0}, std::chrono::nanoseconds{999'999'999});
...
inline auto __io_uring_enter(
    ...
    int __rc = static_cast<int>(::syscall(
        __NR_io_uring_enter, __ring_fd, __to_submit, __min_complete, __flags, __arg,
        __argsz));
    if (__rc == -1) [[__unlikely__]] { return -errno; }
    return __rc;
}
...
[[__gnu__::__always_inline__, __gnu__::__artificial__, __nodiscard__]]
inline auto
__io_uring_register(int __ring_fd, unsigned __opcode, void *__arg, unsigned __nr) noexcept
    -> int {
...
inline void __io_uring_register_eventfd_async(int __rfd, int __efd) {
    int __rc = static_cast<int>(::syscall(
        SYS_io_uring_register, __rfd, IORING_REGISTER_EVENTFD_ASYNC, &__efd, 1));
...
inline auto __map_region(int __fd, ::off_t __offset, std::size_t __size)
    ...
    void *__ptr = ::mmap(
        nullptr, __size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, __fd,
        __offset);
...
struct __completion_queue {
    ...
    explicit __completion_queue(
        const __memory_mapped_region &__cqe, const ::io_uring_params &__p) noexcept
        : __head_{*__at_offset_as<__u32 *>(__cqe.__data_, __p.cq_off.head)},
          __tail_{*__at_offset_as<__u32 *>(__cqe.__data_, __p.cq_off.tail)},
          __cqe_{__at_offset_as<::io_uring_cqe *>(__cqe.__data_, __p.cq_off.cqes)},
          __mask_{*__at_offset_as<__u32 *>(__cqe.__data_, __p.cq_off.ring_mask)} {}
...
struct __scheduler_base {
    ...
    explicit __scheduler_base(unsigned __entries, unsigned __flags)
        ...
        auto __n_sq = __params_.sq_off.array + __params_.sq_entries * sizeof(unsigned);
        auto __n_cq =
            __params_.cq_off.cqes + __params_.cq_entries * sizeof(::io_uring_cqe);
        ...
        __sq_region_  = __map_region(__ringfd_.__fd_, IORING_OFF_SQ_RING, __n_sq);
        ...
        if (__stop) {
            ...
            while (!__tmp._M_empty()) {
                ...
                case _Sg::_S_schedule:
                    ...
                    break;
                case _Sg::_S_stop: {
                    auto *__target = static_cast<__timer_task *>(__op->__target_);
        ...
        if (__state_.compare_exchange_strong(
            ...
            if (__timeout) {
                ...
                __rc = __io_uring_enter(
                    ...
                    IORING_ENTER_GETEVENTS | IORING_ENTER_EXT_ARG, &__arg, sizeof(__arg));
            } else {
                __lock.unlock();
                __rc = __io_uring_enter(
                    __ringfd_.__fd_, __n_newly_submit_, 1, IORING_ENTER_GETEVENTS);
            }
            __state_.store(_Sg::_S_run, std::memory_order_relaxed);
            __throw_error_code_if(__rc < 0 && __rc != -EINTR && __rc != -ETIME, -__rc);
            if (-__rc == ETIME) {
                __n_newly_submit_ = 0;

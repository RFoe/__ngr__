...
struct __frame {
  private:
    ...
    _S_align_up(const std::size_t __val, const std::size_t __align) noexcept
        -> std::size_t {
    ...
    [[__gnu__::__always_inline__, __gnu__::__artificial__]] //
    inline static auto
    _S_align_up_ptr(const char *__ptr, const std::size_t __align) noexcept
        -> char * {
    ...
    void _M_allocate_chunk(const std::size_t __min_usable) {
        const std::size_t __prev_cap =
        ...
        const std::size_t __n   = sizeof(__descriptor) + __new_cap;
        void             *__mem = ::operator new(__n);
  ...
  public:
    ...
    auto _M_push(
        ...
        auto __try_push = [&] constexpr noexcept -> void * {
